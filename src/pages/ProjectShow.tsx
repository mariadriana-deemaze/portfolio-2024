...
export default function ProjectShow({ project, html = '' }: { project?: Project; html?: string }): JSX.Element {
  if (!project) {
    return (
      <div>
  ...
  return (
    <div className="mx-auto w-full max-w-2xl space-y-4">
      <p className="font-mono text-sm text-gray-500">
        <a href="/work" className="hover:underline">&larr; Back to projects</a>
      </p>
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          {technologies.map(({ label, icon }) => {
            const resolvedIcon = isValidElement(icon)
              ? icon
              : Object.values(STACKS).find((t) => t.label === label)?.icon ?? null
